...
constexpr std::string_view uidPrefix = "moe.uwucocoa.rilliya.virtual.";
...
std::string EndpointRegistry::visibleDeviceUID(const EndpointDefinition& endpoint) {
  return std::string(uidPrefix) + identifierString(endpoint.identifier) + "." +
         directionComponent(endpoint.direction);
}
...
std::string EndpointRegistry::companionDeviceUID(const EndpointDefinition& endpoint) {
  ...
  return std::string(uidPrefix) + identifierString(endpoint.identifier) + ".internal." + role;
}
