...
struct ObjectContext final {
  ...
  [[nodiscard]] std::string displayName() const {
    ...
    return "Rilliya Internal " + endpoint.definition.name +
...
public:
  [[nodiscard]] OSStatus initialize(AudioServerPlugInHostRef host) {
    ...
    if (host->CopyFromStorage != nullptr &&
        host->CopyFromStorage(host, catalogStorageKey, &stored) == noErr && stored != nullptr) {
  ...
  [[nodiscard]] OSStatus setCatalog(CFPropertyListRef propertyList) {
    ...
    if (host != nullptr && host->WriteToStorage != nullptr) {
      storageStatus = host->WriteToStorage(host, catalogStorageKey, persisted);
    }
    CFRelease(persisted);
...
OSStatus getPropertyData(AudioServerPlugInDriverRef driver, AudioObjectID objectID, pid_t,
  ...
  if (context->isDevice()) {
    ...
    case kAudioDevicePropertyModelUID:
      return writeCFString(dataSize, outputDataSize, outputData, modelUID);
    case kAudioDevicePropertyTransportType:
...
extern "C" void* RilliyaVirtualAudioDriverFactory(CFAllocatorRef, CFUUIDRef requestedTypeUUID) {
